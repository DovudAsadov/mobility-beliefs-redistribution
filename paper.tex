...
has not exploited the ten-step wealth ladder, which yields a continuous expected
...
introduce a fairness-belief moderator built on the survey's ``most important factor for
...
\begin{enumerate}[leftmargin=28pt,itemsep=2pt,label=\textit{RQ\arabic*.}]
    \item Across the 37 economies, does a higher belief in upward
...
injustice (constructed from question Q4.06). This interaction approach follows
\citet{alesina2005fairness, benabou2006belief} and \citet{alesina2018intergenerational}.

\textbf{Variables.} \emph{Outcomes} (codebook): (a) Q4.01f, agreement that ``the
...
drawn from the household roster using the random-respondent code, following the
technical report.

\textbf{Robustness layer.} The base specification is supplemented with five pre-specified
checks: (i) population-weighted WLS using the \texttt{weight\_pop} variable for
...
re-estimates the main specification using the \texttt{weight\_pop} weights to
...
reproducible from the public \texttt{.dta} file using the scripts in
...
(\cref{tab:main}; the country-level pattern is shown in \cref{fig:country}). The
...
four-year ladder gain ($\beta=-0.049$, $p<0.001$)---while expected ladder mobility
carries a negative but insignificant coefficient ($\beta=-0.015$, $p=0.24$).
...
five-region grouping; dotted lines mark the sample means.}
\label{fig:country}
\end{figure}
...
interaction is positive ($\pi=+0.069$, $p<0.01$)---effort-believers steepen the
attitudinal slope---while the \emph{injustice} interaction is negative
($\pi=-0.077$, $p<0.001$), flattening and at the margin reversing it. For expected
...
belief, by fairness belief. Country-fixed-effects OLS with country-clustered SEs;
...
Exploratory heterogeneity tests confirm context-dependence. Across the five regions
