...
\begin{enumerate}[leftmargin=18pt,itemsep=2pt]
    \item Estimate the partial association between six mobility-belief measures
    (attitudinal items on parents-comparison, four-year retrospective, and prospective
    children, plus three ladder-based measures of current, expected, and realised
    four-year ladder mobility) and two redistribution outcomes, using OLS with country
    fixed effects and country-clustered standard errors.
...
gap between the rich and the poor should be reduced'' on a 1--5 Likert (primary, pro-redistribution coded), and (b) Q4.05e, a binary willingness to pay more taxes for poverty
...
better life''), and (iii) expected ladder mobility = Q2.37 $-$ Q2.35, the difference
between expected position four years ahead and current position on the 10-step wealth
ladder \citep{cantril1965pattern, deaton2008income}. A retrospective ladder change
(Q2.35 $-$ Q2.36) is also computed. \emph{Fairness moderator}: a binary effort indicator
...
relies on selection-on-observables, and results should be interpreted as conditional
...
in terms of associations rather than causal effects. (4) The dual-channel structure of
$H_{1}$ and the asymmetric moderation in $H_{2}$ are grounded in prior theory
\citep{benabou2001social, hirschman1973changing, alesina2005fairness} but were also
sharpened by exploratory inspection of the LiTS IV preliminary estimates. The joint
horse-race specification serves as the confirmatory test and the heterogeneity splits
are reported as exploratory rather than as pre-registered predictions.
...
Czechia, Estonia, Germany, and Greece) and contains 37{,}478 face-to-face interviews
...
region and urban/rural status, (ii) random-walk household selection within each PSU,
and (iii) Kish-grid selection of two adult respondents per household (a knowledgeable
...
portal. No new human-subject data are collected, and the analysis is entirely secondary,
...
attitudinal, three ladder-based) and two redistribution outcomes, where concordant
signs across them strengthen inference. \emph{(ii) Population-weighted WLS} (\cref{tab:weighted})
...
income reduces $N$ from 35{,}766 to about 28{,}000), and a Heckman-type extension is left for
future work.
...
expected ladder mobility carries a negative but insignificant coefficient
($\beta=-0.015$, $p=0.24$).
...
collected and no identifiable individuals are accessed. The survey was administered
under EBRD/World Bank ethical protocols with informed consent and anonymisation handled
at source, and use complies with the EBRD LiTS data licence. The cleaned dataset is
generated from the public \texttt{.dta} file. Replication code is openly available
(Appendix~C) and the raw microdata is not redistributed. The research therefore
qualifies as exempt from institutional ethics review under standard secondary-data
provisions. The author declares no conflicts of interest and no external funding.

Because the design relies on selection-on-observables, all conclusions are framed as
conditional correlations rather than causal effects. Three interpretive risks deserve
explicit acknowledgement: (i) cross-cultural pooling of 37 economies risks treating
heterogeneous concepts (``effort'', ``injustice'', ``redistribution'') as
psychometrically equivalent, which I address with the regional split, (ii) list-wise
deletion on log-income may bias the analytical sample along income-correlated
dimensions, partially offset by population-weighted WLS, and (iii) attitudes measured
in 2022--23 may not generalise to other political climates.
